...
* **複雜度分析：**
  ...
  * 最壞時間複雜度： $O(n^2)$ 
  * 空間複雜度： $O(1)$ 
  * 結論：氣泡排序最大的優點就是「原地排序」，空間複雜度只有 $O(1)$，不需要額外開記憶體。
...
| N | 時間 (秒) | N 增加倍數 | 時間增加倍數 | 理論倍數( $n^2$ )|
| :--- | :--- | :--- | :--- | :--- |
| 1,000 | 0.004 | 基準 | — | 1 |
| 2,000   | 0.017 | 2 | 4.25 | 4 |
| 5,000   | 0.115 | 5 | 28.75 | 25 |
| 10,000   | 0.451 | 10 | 112.75 | 100 |
| 20,000   | 1.854 | 20 | 463.5 | 400 |
| 50,000  | 11.528 | 10 |  2882 | 2500 |
| 100,000 | 46.29 | 100 | 11572 | 10000 |
 
**分析 :** 根據實測數據，氣泡排序的執行時間隨資料量增加的趨勢非常明顯。當資料量 $N$ 從 1,000 增加至 10,000（成長 10 倍）時，執行時間從 0.004 秒增加至 0.458 秒，大約增長了 112.75 倍。這與理論的時間複雜度 $O(n^2)$（$10^2 = 100$ 倍）高度吻合。

**小結 :** 氣泡排序雖然原理直觀、易於實作且具備「原地排序」不佔額外空間的優點，但其頻繁的元素交換導致在實測中耗時較長。實測數據驗證了其 $O(n^2)$ 的特性，對於 $N > 10,000$ 的數據，處理時間會大幅增加，不適合處理大規模資料。
...
| 2,000   | 0.006 | 2 | 2 | 4 |
| 5,000   | 0.036 | 5 | 12 | 25 |
| 10,000   | 0.142 | 10 | 47.34 | 100 |
| 20,000   | 0.568 | 20 | 189.34 | 400 |
| 50,000  | 3.556 | 10 |  1185 | 2500 |
| 100,000 | 15.111 | 100 | 5037 | 10000 |

**分析：** 選擇排序同樣展現了 $O(n^2)$ 的增長特性，但耗時明顯比氣泡排序法好。以 $N=100,000$ 為例，選擇排序僅需約 15.11 秒，而氣泡排序則預計需要約 40 秒。觀察倍數變化，當 $N$ 從 10,000 成長到 100,000（10 倍）時，時間從 0.143 秒增加到 15.11 秒，增加約 105.6 倍，幾乎完美貼合理論預期值。

**小結：** 它每一輪都是先把整排看完、確認誰最小後，才移動到正確位置。儘管氣泡排序和選擇排序在理論複雜度上相同，但後者動手交換的次數少很多，所以處理中大型資料時，速度會快非常多。然而，它與氣泡排序一樣，面對極大規模數據時一樣會跑不動。
...
| 1,000 | 0.002 | 基準 | — | 1 |
| 2,000   | 0.005 | 2 | 2.5 | 4 |
| 5,000   | 0.029 | 5 | 14.5 | 25 |
| 10,000   | 0.112 | 10 | 56 | 100 |
| 20,000   | 0.449 | 20 | 224.5 | 400 |
| 50,000  | 2.85 | 10 |  1425 | 2500 |
| 100,000 | 11.594 | 100 | 5797 | 10000 |

**分析：**  插入排序的執行時間隨 $N$ 增加而平方增長。當資料量從 10,000 增加至 100,000（10 倍）時，執行時間從 0.112 秒增加到 11.594 秒，約為 103.5 倍，這與 $O(n^2)$ 的理論預期（100 倍）極為貼合。

**小結 :** 插入排序是三種基礎排序法（氣泡、選擇、插入）中實測表現最好的。儘管其平均與最壞時間複雜度仍為 $O(n^2)$，但當處理「幾乎已排序完成」的資料時，插入排序能達到接近 $O(n)$ 。
...
| N | 時間 (秒) | N 增加倍數 | 時間增加倍數 | 理論倍數( $nlogn$ )|
| :--- | :--- | :--- | :--- | :--- |
| 1,000 | 0.001 | 基準 | — | 1 |
| 2,000   | 0.002 | 2 | 2 | 2.2 |
| 5,000   | 0.003 | 5 | 3 | 6.16 |
| 10,000   | 0.005 | 10 | 5 | 13.34 |
| 20,000   | 0.009 | 20 | 9 | 28.67 |
| 50,000  | 0.021 | 10 |  21 | 78.3 |
| 100,000 | 0.044 | 100 | 44 | 166.67 |

**分析 :** 當 $N=100,000$ 時，合併排序僅需 0.044 秒，而插入排序也要 11.59 秒。這顯示了 $O(n log n)$ 演算法在處理大數據時的絕對優勢。  在 $N$ 較小時（1,000 到 2,000），時間幾乎沒有變化，這是因為現代處理器速度極快。

**小結 :** 合併排序採用「分治法」策略，將大問題拆解為小問題處理再合併。雖然它需要額外的記憶體空間來存放暫存數組（空間複雜度 $O(n)$），但其穩定的 $O(n log n)$ 表現使其成為實際應用的核心首選。
...
| :--- | :--- | :--- | :--- | :--- |
| 5,000   | 0.001 | 基準 | - | 1 |
| 10,000   | 0.002 | 2 | 2 | 2.1 |
| 20,000   | 0.003 | 4 | 3 | 4.65 |
| 50,000  | 0.009 | 10 |  9 | 12.7 |
| 100,000 | 0.018 | 20 | 18 | 27.03 |

**分析：** 在所有實驗的演算法中，快速排序展現了最快的速度。即使在 $N=100,000$ 的大規模數據下，也僅需0.018 秒即可完成。觀察數據發現，當 $N$ 增加 20 倍時，時間增加約 18 倍，這顯示其時間增長曲線非常接近線性。
**小結 :** 快速排序實測結果為本次實驗中效能最佳的演算法。雖然理論上在極端惡劣的資料分布下會退化至 $O(n^2)$，但在處理一般的隨機資料時，其原地分區的特性能極大化硬體執行效率。實驗數據證明，快速排序在處理數十萬量級的資料時依然游刃有餘，是現代程式庫中最常被採用的通用排序演算法。
...
**數據分析：**
1. 平方增長的規律：在氣泡排序法 $O(n^2)$ 的演算法中，當資料量 $n$ 增加 2 倍時，執行時間都精確地增加了約 4 倍 ($2^2$)。完美驗證了二次方時間複雜度的理論模型。
...
2. **困難與解決**
   ...
   2. 將資料設為10萬筆時，在跑 $O(n^2)$ 演算法（如氣泡排序）視窗會卡住幾秒鐘。一開始以為是程式當掉，後來才發現是因為數據太大，需要時間跑。
